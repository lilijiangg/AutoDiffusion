...
class DPM_Solver:
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type='time_uniform',
        ...
        atol=0.0078, rtol=0.05, ea_timesteps=None,
    ):
        """
        ...
        """
        t_0 = 1. / self.noise_schedule.total_N if t_end is None else t_end  # 0.001
        t_T = self.noise_schedule.T if t_start is None else t_start   # 1.0
        device = x.device
        if method == 'adaptive':
            with torch.no_grad():
            ...
            if ea_timesteps is None:
                timesteps = self.get_time_steps(skip_type=skip_type, t_T=t_T, t_0=t_0, N=steps, device=device) # [1.0000, 0.7502, 0.5005, 0.2508, 0.0010]
            else:
                if max(ea_timesteps) > 1:
                    full_timesteps = self.get_time_steps(skip_type=skip_type, t_T=t_T, t_0=t_0, N=1000, device=device)
                    full_timesteps = list(full_timesteps)
                    full_timesteps.reverse()
                    timesteps = []
                    timesteps += [full_timesteps[ea].item() for ea in ea_timesteps]
                    timesteps = torch.Tensor(timesteps).to('cuda')
                else:
                    timesteps = sorted(ea_timesteps, reverse=True)
                    timesteps = torch.Tensor(timesteps).to('cuda')

                # tmptmp = [ea_timesteps[k] / 1000 * (t_T - t_0) + t_0 for k in range(len(ea_timesteps))] 
                # tmptmp = torch.Tensor(tmptmp)
                # if max(timesteps) > 1:
                #     timesteps = [timesteps[k] / 1000 * (t_T - t_0) + t_0 for k in range(len(timesteps))]
