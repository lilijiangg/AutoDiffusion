...
def main():
    ...
    # total_layer_num = model.layer_num
    # keep_layer_id = [i for i in range(total_layer_num)]
    # random.shuffle(keep_layer_id)
    # skip_layer_id = keep_layer_id[:total_layer_num - args.keep_layer_num]
    ...
    # def model_fn(x, t, y=None, skip_layers=None):
    #     assert y is not None
    #     return model(x, t, y if args.class_cond else None, skip_layer=skip_layers)
    
    def model_fn(x, t, y=None, skip_layers=None, timesteps=None):
        assert y is not None
        t_index = diffusion.timestep_map.index(t[0])
        # t_index = timesteps.index(t[0])
        skip_layer = skip_layers[t_index]
        return model(x, t, y if args.class_cond else None, skip_layer=skip_layer)
    ...
    all_labels = []
    # import pdb
    # pdb.set_trace() # skip_layer_id: [44, 23, 52, 11, 16, 20, 53, 56]
    t1 = time.time()
